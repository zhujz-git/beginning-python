...
with open(filepath, encoding='utf-8') as comb_file:
    ...
    for i in range(color_num):
        #一个组合里面的商品数量
        goods_num = int(re.match(comb_list_pat, comb_file.readline()).group(1))
        goods_id_list = []
        for j in range(goods_num):
            ...
            #根据冒号后面的尺码列表进行拼接
                ...
                goods_id_list.append([tmpline[0]+kclist[k] for k in range(comb_num)])
        #将拼接好的商品编码ID放入每个组合编码后面
        for l in range(comb_num):
            comb_list[(i * comb_num) + l].append([goodsid[l] for goodsid in goods_id_list])
    print(comb_list)