...
def load_user_info(filepath):
    ...
    # 设备地址
    user_addr = first_row.index('住所')
    user_addr_list = sheet1.col_values(user_addr)
    # 设备类别
    user_addr_2 = first_row.index('经营地址')
...
def load_user_phone(filepath, user_info_map):
    try:
        ...
        for i in range(2, nrow + 1):
            ...
            if user_info:
                #0:手机号码 1:住所 2:经营地址
                load_sheet['{}{}'.format('G', i)].value = user_info[0]
                #过滤娄桥之前的地址信息
                user_advice_addr = user_info[1]
                user_advice_addr = re.sub(pat, '', user_advice_addr)
                load_sheet['{}{}'.format('H', i)].value = user_advice_addr
                user_advice_addr = user_info[2]
                user_advice_addr = re.sub(pat, '', user_advice_addr)
                load_sheet['{}{}'.format('I', i)].value = user_advice_addr
            else:
                load_sheet['{}{}'.format('G', i)].value = '未查到该单位'
...
if __name__ == '__main__':
    user_info_map = load_user_info('娄桥市场主体名单1.xls')
    user_info_map.update(load_user_info('娄桥市场主体名单2.xls'))
    load_user_phone('瓯海市场主体“企业在线”移动端未激活名单.xlsx', user_info_map)
